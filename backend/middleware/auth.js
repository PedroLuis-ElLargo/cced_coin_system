...
// (admin puede ver todos, estudiante solo puede ver sus propias calificaciones)
const verifyStudentAccess = async (req, res, next) => {
  try {
    const estudianteId =
      req.params.estudianteId ||
      req.query.estudianteId ||
    ...
    if (req.user.rol === "student" || req.user.rol === "estudiante") {
      if (estudianteId && parseInt(estudianteId) !== req.user.id) {
        return res.status(403).json({
          success: false,
    ...
    console.error("Error en verifyStudentAccess:", error);
    return res.status(500).json({
      success: false,
